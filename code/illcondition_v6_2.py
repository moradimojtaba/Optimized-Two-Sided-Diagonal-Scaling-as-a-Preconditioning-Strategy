...
OUTDIR = Path.cwd() / "illcondition_v6_2_results"
...
# V6.2 scalability design:
# Ruiz is benchmarked to n=1000; optimizer-based methods are extended to n=300
# to demonstrate computational growth without making the full benchmark
# prohibitively expensive on CI runners.
SCALABILITY_RUIZ_SIZES = [100, 300, 600, 1000]
SCALABILITY_OPT_SIZES = [100, 300]
SCALABILITY_LBFGS_MAXITER = 40
...
def ruiz_scale(A):
    ...
    return d1, d2, history, converged


def run_ruiz(name, A, family):
    t0 = time.perf_counter()
    d1, d2, hist, ruiz_converged = ruiz_scale(A)
    ...
    r = result_from_scaling(
        ...
        ruiz_converged,
        "converged" if ruiz_converged else "max_iterations_reached"
    )
...
def run_lbfgs(name, A, family, x0=None, maxiter=LBFGS_MAXITER,
    ...
    try:
        ...
        if not np.isfinite(res.fun):
            status = "failed"
            opt_converged = False
            termination = "nonfinite_objective"
...
def run_hybrid(name, A, family):
    t0 = time.perf_counter()
    d1r, d2r, ruiz_hist, ruiz_converged = ruiz_scale(A)
    ...
    hist["ruiz_converged"] = ruiz_converged
    return r, d1, d2, hist
...
def run_ga(name, A, family, seed):
    ...
    r = result_from_scaling(
        ...
        "Real-coded genetic algorithm with crossover and mutation; fixed-generation run.",
        None, "completed_fixed_generations"
    )
    return r, d1, d2, history
...
def run_bo(name, A, family):
    ...
    try:
        ...
        r = result_from_scaling(
            ...
            "Bayesian optimization; completed budget.",
            None, "completed_fixed_budget"
        )
        hist = {
...
def save_histograms(results_df):
    good = results_df[results_df["status"].isin(
        ["completed", "converged", "not_converged"]
    )].copy()
    if good.empty:
        return
...
def summarize_methods(results_df):
    # A run can finish execution without the optimizer converging.
    # V6.2 therefore keeps both converged and non-converged optimizer runs
    # in the numerical summary and reports convergence explicitly.
    valid_status = {"completed", "converged", "not_converged"}
    valid = results_df[results_df["status"].isin(valid_status)].copy()
    if valid.empty:
        return pd.DataFrame()

    valid["log10_final_cond"] = np.log10(
        valid["final_cond"].replace([np.inf, -np.inf], np.nan)
    )
    valid["is_converged"] = (
    valid["optimization_converged"]
    .astype("boolean")
    .fillna(False)
    .astype(bool)
)
    ...
    summary["convergence_rate"] = np.where(
        summary["runs"] > 0,
        summary["converged_runs"] / summary["runs"],
        np.nan,
    )
    return summary
...
def stochastic_summary(stochastic_df):
    ...
    completed = stochastic_df[stochastic_df["status"].isin(
        ["completed", "converged", "not_converged"]
    )].copy()
    if completed.empty:
        return pd.DataFrame()
...
def write_configuration():
    cfg = {
        ...
        "scalability_ruiz_sizes": SCALABILITY_RUIZ_SIZES,
        "scalability_optimizer_sizes": SCALABILITY_OPT_SIZES,
        "scalability_lbfgs_maxiter": SCALABILITY_LBFGS_MAXITER,
        ...
        "notes": [
            ...
            "Optimizer execution status is separated from optimizer convergence.",
            "Non-converged optimizer runs are retained and explicitly flagged.",
            "L-BFGS convergence is callback/iteration aligned.",
            "Total solver cost = preconditioner setup + median solve time.",
            "Scalability is evaluated across Ruiz and optimizer-based methods.",
        ],
    }
...
def main():
    ...
    # -------------------------------------------------------------------------
    # V6.2 scalability experiment
    # Ruiz is evaluated to n=1000; L-BFGS and Ruiz-LBFGS are evaluated to n=300
    # with a bounded optimizer budget. This explicitly tests computational
    # growth while keeping CI runtime practical.
    # -------------------------------------------------------------------------
    scalability_rows = []

    for n in SCALABILITY_RUIZ_SIZES:
        ...
        scalability_rows.append({
            ...
            "runtime_sec": elapsed,
            "iterations": len(hist),
            "optimization_converged": ruiz_converged,
            "termination_status": (
                "converged" if ruiz_converged else "max_iterations_reached"
            ),
        })

    for n in SCALABILITY_OPT_SIZES:
        A = finite_difference(n)

        # L-BFGS
        t0 = time.perf_counter()
        r, d1, d2, hist = run_lbfgs(
            "FiniteDiff", A, "sparse_spd",
            x0=None,
            maxiter=SCALABILITY_LBFGS_MAXITER,
            method_name="L-BFGS",
        )
        elapsed = time.perf_counter() - t0
        B = apply_scaling(A, d1, d2)
        scalability_rows.append({
            ...
            "optimization_converged": r.optimization_converged,
            "termination_status": r.termination_status,
        })

        # Ruiz -> L-BFGS hybrid
        t0 = time.perf_counter()
        r, d1, d2, hist = run_hybrid(
            "FiniteDiff", A, "sparse_spd"
        )
        elapsed = time.perf_counter() - t0
        B = apply_scaling(A, d1, d2)
    ...
    # V6.2 keeps a single canonical scalability table to avoid duplicate files.
    ...
    report = {
        ...
        "method_status_counts": (
            results_df["status"].value_counts(dropna=False).to_dict()
            if not results_df.empty else {}
        ),
        "optimizer_converged_runs": (
            int(
    results_df["optimization_converged"]
    .astype("boolean")
    .fillna(False)
    .sum()
)
        ...
        "optimizer_nonconverged_runs": (
            int((results_df["optimization_converged"] == False).sum())
            if not results_df.empty else 0
        ),
        "solver_status_counts": (
            solver_df["solver_status"].value_counts(dropna=False).to_dict()
            if not solver_df.empty else {}
        ),
        "pyamg_available": HAVE_PYAMG,
        "skopt_available": HAVE_SKOPT,
        "git_commit": os.environ.get("GITHUB_SHA", "local"),
